...
const SingleCard = ({ category }) => {
  ...
  // Destructure the necessary fields from the category prop
            ...
            <div className="p-3 border bg-light h-100 d-flex" style={{ flexDirection: 'column', justifyContent: 'space-between' }}>
                  ...
                  <Row>
                    {(() => {
                      switch (image.length) {
                        ...
                        default:
                          return <Row key={0} className="image-grid"><img src={image[0]} alt="img" /></Row>;
                      }
                    })()}
                  </Row>
                )}
              ...
              <div className="container w-100 p-2">
                <div className="row flex-nowrap justify-content-evenly align-content-center">
                  <span className="col-2"><BsHandThumbsUp fill={'#808080'} size={25} /></span>
                  <span className="col-2"><FaRegComments fill={'#43a047'} size={25} /></span>
                  <span
                    className="col-2"
                    onClick={() => {
                      if (isBookmarked) {
                        removeItemBookmark({ id });
                      } else {
                        addItemBookmarks({ id, title, author, image, Text, price });
                      }
                    }}
                  >
                    {isBookmarked ? <BsBookmarkDashFill fill={'#4caf50bd'} size={20} /> : <BsBookmarkPlus size={20} />}
                  </span>
                  <span className="col-2"><BsEye fill={'#663399'} size={25} /></span>
                  <span className="col-2"><BsActivity fill={'#ffa000'} size={25} /></span>
                  <span className="col-2"><FcShare size={25} /></span>
                </div>
              </div>
            </div>
          </div>
